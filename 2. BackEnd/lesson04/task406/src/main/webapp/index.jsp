...
<c:if test="${paramValues.error[0] eq '1'}">
    <% if (true) throw new RuntimeException("Exception has been raised");%>
</c:if>
...
The Server is running on port ${pageContext.request.serverPort} <br>
Servlet Name is ${pageContext.servletConfig.servletName} <br>
Server Info: ${pageContext.servletContext.serverInfo} <br>

Page name: ${pageScope["javax.servlet.jsp.jspPage"]} <br>

<c:set var="userid" scope="page" value="John Doe" />
userId attribute from pageContext: ${pageScope.userid} <br>
